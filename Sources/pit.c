...
{
//    __VECTOR_RAM[INT_PIT0_PIF + u8PITModule] = (uint32)pit_isr_ptr;
	__VECTOR_RAM[64+55] = (uint32)pit_isr_ptr;
	
    /* Enable Interrupts for PIT0 (55) and PIT1(56) */
    MCF_INTC0_ICR55 = MCF_INTC_ICR_IL(4)|MCF_INTC_ICR_IP(7);
    ...
    MCF_PIT_PCSR(u8PITModule) |= MCF_PIT_PCSR_PIF;
...
uint32 PIT_SetFreq (uint32 u32FreqHz, uint8 u8PITModule) 
{
    ...
    for (i=0; i < 16; i++){
        u32Modulus = (SYSTEM_CLOCK_KHZ*1000/2) / (u32FreqHz*pow(i));
        if (u32Modulus < 65535){
            ...
            return ((SYSTEM_CLOCK_KHZ*1000/2) / (pow(i)*u32Modulus));
        }
    }
    return -1;
}
...
uint32 PIT_SetPeriod (uint32 u32Periodms, uint8 u8PITModule) 
{
    ...
    for (i=0; i < 16; i++){
        u32Modulus = ((SYSTEM_CLOCK_KHZ/2) * u32Periodms) / pow(i);
        if (u32Modulus < 65535){
            /* Try to find a good prescaler */
            MCF_PIT_PCSR(u8PITModule) &= ~MCF_PIT_PCSR_PRE(0x0F);
            MCF_PIT_PCSR(u8PITModule) |= MCF_PIT_PCSR_PRE(i);
            MCF_PIT_PMR(u8PITModule) = u32Modulus;
            return ((pow(i)*u32Modulus) / (SYSTEM_CLOCK_KHZ/2));
        }
    }
    return -1;  
}
